...
bool IsSysServiceExpired(const uint64_t &nTime)
{
	if(!chainActive.Tip())
		return false;
	return (chainActive.Tip()->GetMedianTimePast() >= (int64_t)nTime);
...
{   
	if (IsPaymentOptionInMask(paymentOptionMask, PAYMENTOPTION_BTC))
		return 250;
	else  if (IsPaymentOptionInMask(paymentOptionMask, PAYMENTOPTION_DYN))
		return 25;
	else  if (IsPaymentOptionInMask(paymentOptionMask, PAYMENTOPTION_SEQ))
		return 25;
	return 25;
}
...
bool CheckAliasInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, bool fJustCheck, int nHeight, string &errorMessage, bool &bDestCheckFailed, bool dontaddtodb) {
	...
	{
		errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 5003 - " + _("Too many outputs for this Syscoin transaction");
		return error(errorMessage.c_str());
	}
	...
	if(fJustCheck || op != OP_ALIAS_ACTIVATE)
	{
		...
		for (unsigned int i = 0; i < tx.vin.size(); i++) {
			...
			Coin prevCoins;
			GetUTXOCoin(tx.vin[i].prevout, prevCoins);

			// ensure inputs are unspent when doing consensus check to add to block
			if(!prevCoins.IsNull() || !IsSyscoinScript(prevCoins.vout[tx.vin[i].prevout.n].scriptPubKey, pop, vvch))
			{
				continue;
			}
		...
		if(vvchArgs.size() >= 4 && !vvchArgs[3].empty())
		{
			...
			if(!bWitnessSigFound)
			{
				errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 5002 - " + _("Witness signature not found");
				return error(errorMessage.c_str());
			}
		}
	}
	...
	if(fJustCheck)
	{
		...
		{
			errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 3006 - " + _("Encryption private key too long");
			return error(errorMessage.c_str());
		}
		if(theAlias.vchEncryptionPublicKey.size() > MAX_GUID_LENGTH)
		{
			errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 3006 - " + _("Encryption public key too long");
			return error(errorMessage.c_str());
		}
		switch (op) {
			case OP_ALIAS_ACTIVATE:
				...
				{
					errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 5009 - " + _("Alias input to this transaction not found");
					return error(errorMessage.c_str());
				}
				// Check new/activate hash
				if (vvchPrevArgs.size() <= 0 || vvchPrevArgs[0] != vchHash)
				{
					errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 5010 - " + _("Alias new and activate hash mismatch");
					return error(errorMessage.c_str());
				}
				...
				{
					errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 5011 - " + _("Alias address cannot be empty");
					return error(errorMessage.c_str());
				}
				break;
			case OP_ALIAS_UPDATE:
	...
	if (!fJustCheck) {
		...
		if (!vchData.empty() && theAlias.offerWhitelist.entries.empty() && theAlias.nExpireTime > 0)
		{
			...
			{
				errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 5019 - " + _("Transaction does not pay enough fee: ") + ValueFromAmount(tx.vout[nDataOut].nValue).write() + "/" + ValueFromAmount(fee - 10000).write() + "/" + boost::lexical_cast<string>(fYears) + " years.";
				return true;
			}
		}
			...
			if (!theAliasNull) {
				...
				else
				{
					...
					if (dbAlias.vchAddress != theAlias.vchAddress)
					{
						...
						{
							errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 5026 - " + _("Cannot edit this alias. Insufficient privileges.");
							theAlias = dbAlias;
						}
					...
					else
					{
						...
						{
							errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 5026 - " + _("Cannot edit this alias. It is view-only.");
							theAlias = dbAlias;
						}
					}
					if (theAlias.nAccessFlags > dbAlias.nAccessFlags)
					{
						errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot modify for more lenient access. Only tighter access level can be granted.");
						theAlias = dbAlias;
					}
				}
				...
				if (whiteList.entries.size() >= 1)
				{
					...
					{
						errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 1094 -" + _("Too many affiliates for this whitelist, maximum 20 entries allowed");
					...
					else if (whiteList.entries.size() == 1 && whiteList.entries.begin()->second.nDiscountPct == 127)
					{
						...
						{
							errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 1093 - " + _("Whitelist is already empty");
						}
						else
							theAlias.offerWhitelist.SetNull();
					}
					else
					{
						for (auto const &it : whiteList.entries)
						{
							...
							if (newEntry.nDiscountPct > 99) {
								errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 1094 -" + _("Whitelist discount must be between 0 and 99");
								continue;
							}
		...
		else if (op == OP_ALIAS_ACTIVATE)
		{
			...
			if (paliasdb->ExistsAddress(theAlias.vchAddress) && chainActive.Tip()->GetMedianTimePast() < (int64_t)GetAliasExpiration(theAlias))
			{
				errorMessage = "DYNAMIC_ALIAS_CONSENSUS_ERROR: ERRCODE: 5028 - " + _("Trying to create an alias with an address of an alias that isn't expired");
				return true;
			}
		}
		if (!theAliasNull)
...
void GetAddress(const CAliasIndex& alias, CDynamicAddress* address,CScript& script,const uint32_t nPaymentOption)
{
	...
	CDynamicAddress addrTmp = CDynamicAddress(EncodeBase58(alias.vchAddress));
	address[0] = CDynamicAddress(addrTmp.Get(), myAddressType);
        ...
        try {
			if (pcursor->GetKey(key) && key.first == "namei") {
				pcursor->GetValue(txPos);
  				if (chainActive.Tip()->GetMedianTimePast() >= (int64_t)txPos.nExpireTime)
				{
					servicesCleaned++;
				...
				if (GetAlias(value, alias) && chainActive.Tip()->GetMedianTimePast() >= (int64_t)alias.nExpireTime)
				{
					servicesCleaned++;
...
bool BuildAliasIndexerTxHistoryJson(const string &user1, const string &user2, const string &user3, const uint256 &txHash, const uint64_t& nHeight, const string &type, const string &guid, UniValue& oName)
{
	...
	int64_t nTime = 0;
	if (chainActive.Height() >= (int)nHeight) {
...
UniValue syscoinquery(const UniValue& params, bool fHelp) {
	...
	if (collection == "alias")
		...
		throw runtime_error("DYNAMIC_ALIAS_RPC_ERROR: ERRCODE: 5508 - " + _("Invalid selection collection name, please specify the collection parameter as either 'alias', 'cert', 'offer', 'feedback' or 'escrow'"));
	...
	try {
		...
		if (!filter) {
			throw runtime_error("DYNAMIC_ALIAS_RPC_ERROR: ERRCODE: 5505 - " + boost::lexical_cast<string>(error.message));
		}
	}
	catch (...) {
		throw runtime_error("DYNAMIC_ALIAS_RPC_ERROR: ERRCODE: 5508 - " + _("Cannot convert query into JSON"));
	}
	if (!options.empty()) {
		opts = bson_new_from_json((const uint8_t *)options.c_str(), -1, &error);
		if (!opts) {
			bson_destroy(filter);
			throw runtime_error("DYNAMIC_ALIAS_RPC_ERROR: ERRCODE: 5506 - " + boost::lexical_cast<string>(error.message));
		}
	}
...
bool BuildAliasJson(const CAliasIndex& alias, UniValue& oName)
{
	...
	int64_t nTime = 0;
	if (chainActive.Height() >= (int)alias.nHeight) {
