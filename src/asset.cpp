	...
	while (pcursor->Valid()) {
		...
		try {
			if (pcursor->GetKey(key) && key.first == "asseti") {
				if (!GetAsset(key.second, txPos) || chainActive.Tip()->GetMedianTimePast() >= (int64_t)GetAssetExpiration(txPos))
				{
					servicesCleaned++;
...
bool CheckAssetInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, const std::vector<unsigned char> &vvchAlias,
	...
	{
		errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR ERRCODE: 2001 - " + _("Cannot unserialize data inside of this transaction relating to a asset");
		return true;
	}
	...
	if(fJustCheck)
	{
		if(vvchArgs.size() != 1)
		{
			errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2002 - " + _("Asset arguments incorrect size");
		...
		{
			errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2003 - " + _("Hash provided doesn't match the calculated hash of the data");
			return true;
		}
	...
	if(fJustCheck)
	{
		if (op != OP_ASSET_SEND) {
			...
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2005 - " + _("Asset category too big");
				return error(errorMessage.c_str());
			}
			if (theAsset.vchPubData.size() > MAX_VALUE_LENGTH)
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2007 - " + _("Asset public data too big");
				return error(errorMessage.c_str());
			}
		}
		switch (op) {
		case OP_ASSET_ACTIVATE:
			...
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2004 - " + _("asset hex guid too long");
				return error(errorMessage.c_str());
			}
			if(!boost::algorithm::starts_with(stringFromVch(theAsset.sCategory), "assets"))
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2013 - " + _("Must use a asset category");
				return true;
			}
			break;
		...
		case OP_ASSET_UPDATE:
			...
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2017 - " + _("Must use a asset category");
				return true;
			}
			if (theAsset.nBalance < 0)
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2015 - " + _("Balance must be greator than or equal to 0");
				return error(errorMessage.c_str());
			}
			break;
		...
		case OP_ASSET_SEND:
			...
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2021 - " + _("Asset send must send an input or transfer balance");
				return error(errorMessage.c_str());
			}
			break;
		default:
			errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2021 - " + _("Asset transaction has unknown op");
			return error(errorMessage.c_str());
		}
	}
	if (!fJustCheck) {
		...
		if (!GetAsset(op == OP_ASSET_SEND ? theAssetAllocation.vchAsset : theAsset.vchAsset, dbAsset))
		{
			if (op != OP_ASSET_ACTIVATE) {
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2022 - " + _("Failed to read from asset DB");
				return true;
			}
		}
		...
		if (op == OP_ASSET_UPDATE || op == OP_ASSET_TRANSFER || op == OP_ASSET_SEND)
		{
			...
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot edit this asset. Asset owner must sign off on this change");
				return true;
			}
		}
		if (op == OP_ASSET_UPDATE) {
			...
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Balance cannot exceed total supply");
				return true;
			}
		}
		...
		if (op == OP_ASSET_SEND) {
			...
			if (theAssetAllocation.listSendingAllocationInputs.empty()) {
				if (!dbAssetAllocation.listAllocationInputs.empty()) {
					errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Invalid asset send, request not sending with inputs and sender uses inputs in its allocation list");
					return true;
				}
				if (theAssetAllocation.listSendingAllocationAmounts.empty()) {
					errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Invalid asset send, expected allocation amounts");
					return true;
				}
				...
				for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
					...
					{
						errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Receiving amount must be positive");
						return true;
					}
				}
				if (theAsset.nBalance < nTotal) {
					errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Sender balance is insufficient");
					return true;
				}
				for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
					...
					{
						errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2024 - " + _("Cannot find alias you are transferring to.");
						continue;
					}
					if (!(alias.nAcceptTransferFlags & ACCEPT_TRANSFER_ASSETS))
					{
						errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("An alias you are transferring to does not accept assets");
						continue;
					}
					...
					if (!dontaddtodb) {
						...
						{
							errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2028 - " + _("Failed to revert asset allocation");
							return error(errorMessage.c_str());
						}
						...
						{
							errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2028 - " + _("Failed to write to asset allocation DB");
							continue;
						}
						if (strResponse != "") {
		...
		else if (op != OP_ASSET_ACTIVATE)
		{
			...
			if (op == OP_ASSET_TRANSFER)
			{
				...
				if (!(alias.nAcceptTransferFlags & ACCEPT_TRANSFER_ASSETS))
				{
					errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("The alias you are transferring to does not accept assets");
					return true;
				}
			}
		}
		if (op == OP_ASSET_ACTIVATE)
		{
			...
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2027 - " + _("Asset already exists");
				return true;
			}
		}
		if (!dontaddtodb) {
		...
		if (!dontaddtodb) {
			...
			{
				errorMessage = "DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2028 - " + _("Failed to write to asset DB");
				return error(errorMessage.c_str());
			}
			// debug
			if (fDebug)
...
UniValue assetnew(const UniValue& params, bool fHelp) {
	...
	if (!GetAlias(vchAlias, theAlias))
		throw runtime_error("DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2500 - " + _("failed to read alias from alias DB"));
    ...
    scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_ASSET) << CScript::EncodeOP_N(OP_ASSET_ACTIVATE) << vchHashAsset << OP_2DROP << OP_DROP;
    scriptPubKey += scriptPubKeyOrig;
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_DYNAMIC_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << theAlias.vchAlias << theAlias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
...
UniValue assetupdate(const UniValue& params, bool fHelp) {
    ...
    if (!GetAsset( vchAsset, theAsset))
        throw runtime_error("DYNAMIC_ASSET_RPC_ERROR: ERRCODE: 2504 - " + _("Could not find a asset with this key"));

	CAliasIndex theAlias;

	if (!GetAlias(theAsset.vchAlias, theAlias))
		throw runtime_error("DYNAMIC_ASSET_CONSENSUS_ERROR: ERRCODE: 2505 - " + _("Failed to read alias from alias DB"));
    ...
    vector<unsigned char> vchHashAsset = vchFromValue(hash.GetHex());
    scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_ASSET) << CScript::EncodeOP_N(OP_ASSET_UPDATE) << vchHashAsset << OP_2DROP << OP_DROP;
    scriptPubKey += scriptPubKeyOrig;
...
UniValue assettransfer(const UniValue& params, bool fHelp) {
 if (fHelp || params.size() != 3)
	...
	if (!GetAlias(vchAlias, toAlias))
		throw runtime_error("DYNAMIC_ASSET_RPC_ERROR: ERRCODE: 2509 - " + _("Failed to read transfer alias from DB"));
	...
	CAsset theAsset;
    if (!GetAsset( vchAsset, theAsset))
        throw runtime_error("DYNAMIC_ASSET_RPC_ERROR: ERRCODE: 2510 - " + _("Could not find a asset with this key"));

	CAliasIndex fromAlias;
	if(!GetAlias(theAsset.vchAlias, fromAlias))
	{
		 throw runtime_error("DYNAMIC_ASSET_RPC_ERROR: ERRCODE: 2511 - " + _("Could not find the asset alias"));
	}
    ...
    vector<unsigned char> vchHashAsset = vchFromValue(hash.GetHex());
    scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_ASSET) << CScript::EncodeOP_N(OP_ASSET_TRANSFER) << vchHashAsset << OP_2DROP << OP_DROP;
	scriptPubKey += scriptPubKeyOrig;
	...
	CScript scriptPubKeyAlias;
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_DYNAMIC_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << fromAlias.vchAlias << fromAlias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
...
UniValue assetsend(const UniValue& params, bool fHelp) {
	...
	if (!GetAlias(vchAliasFrom, fromAlias))
		throw runtime_error("DYNAMIC_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 2509 - " + _("Failed to read transfer alias from DB"));
	...
	CAsset theAsset;
	if (!GetAsset(vchAsset, theAsset))
		throw runtime_error("DYNAMIC_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 2510 - " + _("Could not find a asset with this key"));
	...
	vector<unsigned char> vchHashAsset = vchFromValue(hash.GetHex());
	scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_ASSET) << CScript::EncodeOP_N(OP_ASSET_SEND) << vchHashAsset << OP_2DROP << OP_DROP;
...
UniValue assetinfo(const UniValue& params, bool fHelp) {
	...
	CAsset txPos;
	if (!passetdb || !passetdb->ReadAsset(vchAsset, txPos))
		throw runtime_error("DYNAMIC_ASSET_RPC_ERROR: ERRCODE: 5536 - " + _("Failed to read from asset DB"));
