...
bool DecodeAssetAllocationScript(const CScript& script, int& op,
	...
	if (op != OP_DYNAMIC_ASSET_ALLOCATION)
		return false;
...
bool RevertAssetAllocation(const CAssetAllocationTuple &assetAllocationToRemove, const uint256 &txHash, sorted_vector<CAssetAllocationTuple> &revertedAssetAllocations) {
	...
	{
		errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2022 - " + _("Failed to write to asset allocation DB");
...
bool CheckAssetAllocationInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, const std::vector<unsigned char> &vchAlias,
	...
	{
		errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR ERRCODE: 2001 - " + _("Cannot unserialize data inside of this transaction relating to a assetallocation");
		return true;
	}
	...
	if(fJustCheck)
	{
		if(vvchArgs.size() != 1)
		{
			errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2002 - " + _("Asset arguments incorrect size");
			return error(errorMessage.c_str());
		}		
		if(vchHash != vvchArgs[0])
		{
			errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2003 - " + _("Hash provided doesn't match the calculated hash of the data");
			return true;
		}		
	}
	...
	if(fJustCheck)
	{
		...
		case OP_ASSET_ALLOCATION_SEND:
			...
			{
				errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2021 - " + _("Asset send must send an input or transfer balance");
				return error(errorMessage.c_str());
			}
			break;

		default:
			errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2021 - " + _("Asset transaction has unknown op");
			return error(errorMessage.c_str());
		}
	}
	...
	if (op == OP_ASSET_ALLOCATION_SEND)
	{
		if (!dontaddtodb) {
			...
			if (bRevert) {
				...
				{
					errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2028 - " + _("Failed to revert asset allocation");
					return error(errorMessage.c_str());
				}
			}
		}
		...
		{
			errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2024 - " + _("Cannot find sender asset allocation.");
			return true;
		}
		...
		{
			errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot send this asset. Asset allocation owner must sign off on this change");
			return true;
		}
		if (theAssetAllocation.listSendingAllocationInputs.empty()) {
			if (!dbAssetAllocation.listAllocationInputs.empty()) {
				errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Invalid asset send, request not sending with inputs and sender uses inputs in its allocation list");
				return true;
			}
			if (theAssetAllocation.listSendingAllocationAmounts.empty()) {
				errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Invalid asset send, expected allocation amounts");
				return true;
			}
			...
			for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
				...
				{
					errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Receiving amount must be positive");
					return true;
				}
			}
			if (dbAssetAllocation.nBalance < nTotal) {
				errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Sender balance is insufficient");
			...
			for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
				...
				if (amountTuple.first == vchAlias) {
					errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Cannot send an asset allocation to yourself");
					continue;
				}
				const CAssetAllocationTuple receiverAllocationTuple(theAssetAllocation.vchAsset, amountTuple.first);
				if (fJustCheck) {
					...
					{
						errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2024 - " + _("Cannot find alias you are transferring to");
						continue;
					}
					if (!(alias.nAcceptTransferFlags & ACCEPT_TRANSFER_ASSETS))
					{
						errorMessage = "DYNAMIC_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("An alias you are transferring to does not accept assets");
						continue;
					}
...
UniValue assetallocationsend(const UniValue& params, bool fHelp) {
	...
	if (!GetBoolArg("-unittest", false)) {
		...
		for (auto& arrivalTime : arrivalTimes) {
			...
			if ((nNow - (arrivalTime.second / 1000)) < ZDAG_MINIMUM_LATENCY_SECONDS) {
				throw runtime_error("DYNAMIC_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 2510 - " + _("Please wait a few more seconds and try again..."));
			}
		}
	}
	if (assetAllocationConflicts.find(assetAllocationTuple) != assetAllocationConflicts.end())
		throw runtime_error("DYNAMIC_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 2510 - " + _("This asset allocation is involved in a conflict which must be resolved with Proof-Of-Work. Please wait for a block confirmation and try again..."));
	...
	scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_ASSET_ALLOCATION) << CScript::EncodeOP_N(OP_ASSET_ALLOCATION_SEND) << vchHashAsset << OP_2DROP << OP_DROP;
...
UniValue assetallocationinfo(const UniValue& params, bool fHelp) {
	...
	if (!passetallocationdb || !passetallocationdb->ReadAssetAllocation(CAssetAllocationTuple(vchAsset, vchAlias), txPos))
		throw runtime_error("DYNAMIC_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 5536 - " + _("Failed to read from assetallocation DB"));
