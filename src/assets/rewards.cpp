...
/** ASSET END */
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for DYN
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using DYN differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "DYN") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Dynamic address (already validated during ownership snapshot creation)
