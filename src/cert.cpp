...
bool RevertCert(const std::vector<unsigned char>& vchCert, const int op, const uint256 &txHash, sorted_vector<std::vector<unsigned char> > &revertedCerts) {
	...
	if (!pcertdb->ReadLastCert(vchCert, dbCert)) {
		...
		{
			errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2022 - " + _("Failed to erase cert");
	...
	{
		errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2022 - " + _("Failed to write to cert DB");
		return error(errorMessage.c_str());
	}
...
bool CheckCertInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, const std::vector<unsigned char> &vvchAlias,
	...
	if (!fJustCheck && !dontaddtodb) {
		...
		{
			errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2028 - " + _("Failed to revert cert");
			return error(errorMessage.c_str());
		}
	}
	...
	if(op != OP_CERT_ACTIVATE) 
	{
		...
		{
			errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot update this certificate. Certificate owner must sign off on this change.");
			return true;
		}
		...
		if (!GetFirstCert(dbCert.vchCert, firstCert)) {
			errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("Cannot read first cert from cert DB");
			return true;
		}
		if(op == OP_CERT_TRANSFER)
		{
			...
			{
				errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot transfer this certificate. Insufficient privileges.");
				return true;
			}
		}
		else if(op == OP_CERT_UPDATE)
		{
			...
			{
				errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot edit this certificate. It is view-only.");
				return true;
			}
		}
		if(theCert.nAccessFlags > dbCert.nAccessFlags && dbCert.vchAlias != firstCert.vchAlias)
		{
			errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot modify for more lenient access. Only tighter access level can be granted.");
			return true;
		}
	}
	else
...
UniValue certnew(const UniValue& params, bool fHelp) {
    ...
    scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_CERT) << CScript::EncodeOP_N(OP_CERT_ACTIVATE) << vchHashCert << OP_2DROP << OP_DROP;
...
UniValue certupdate(const UniValue& params, bool fHelp) {
	...
	if (!GetBoolArg("-unittest", false)) {
		...
		for (auto& arrivalTime : arrivalTimes) {
			...
			if ((nNow - (arrivalTime.second / 1000)) < ZDAG_MINIMUM_LATENCY_SECONDS) {
				throw runtime_error("DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2510 - " + _("Please wait a few more seconds and try again..."));
			}
		}
	}
    ...
    vector<unsigned char> vchHashCert = vchFromValue(hash.GetHex());
    scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_CERT) << CScript::EncodeOP_N(OP_CERT_UPDATE) << vchHashCert << OP_2DROP << OP_DROP;
    scriptPubKey += scriptPubKeyOrig;
...
UniValue certtransfer(const UniValue& params, bool fHelp) {
 if (fHelp || params.size() != 5)
    ...
    vector<unsigned char> vchHashCert = vchFromValue(hash.GetHex());
    scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_CERT) << CScript::EncodeOP_N(OP_CERT_TRANSFER) << vchHashCert << OP_2DROP << OP_DROP;
	scriptPubKey += scriptPubKeyOrig;
...
UniValue certinfo(const UniValue& params, bool fHelp) {
	...
	CCert txPos;
	if (!pcertdb || !pcertdb->ReadCert(vchCert, txPos))
		throw runtime_error("DYNAMIC_CERT_RPC_ERROR: ERRCODE: 5536 - " + _("Failed to read from cert DB"));
