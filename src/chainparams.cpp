...
static CBlock CreateGenesisBlock(const char* pszTimestamp, const CScript& genesisOutputScript, const uint32_t nTime, const uint32_t nNonce, const uint32_t nBits, const int32_t nVersion, const CAmount& genesisReward)
{
    ...
    txNew.vin.resize(1);
    txNew.vout.resize(1);
    txNew.vin[0].scriptSig = CScript() << 1485090000 << CScriptNum(4) << std::vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
{
    const char* pszTimestamp = "NY Times: Hints of a Sustained Campaign of Protest";
...
public:
    CMainParams() {
        ...
        genesis = CreateGenesisBlock(1485098638, 18648, UintToArith256(consensus.powLimit).GetCompact(), 1, (1 * COIN));
        if(startNewChain == true) { MineGenesis(genesis, consensus.powLimit, true); }

        consensus.hashGenesisBlock = genesis.GetHash();

        if(!startNewChain)
            assert(consensus.hashGenesisBlock == uint256S("0x0000c88a02e6cfdb833f4b9ab3728632e733dfae0301c9e4fede337d087f8e17"));
            assert(genesis.hashMerkleRoot == uint256S("0x9063f90d0c26cd91838880e3d00a8c9ba4d92c6d224b7f7e8475ba42a680df44"));
        ...
        checkpointData = (CCheckpointData) {
            ...
            (  0, uint256S("0x0000c88a02e6cfdb833f4b9ab3728632e733dfae0301c9e4fede337d087f8e17")),
            1485098638, // * UNIX timestamp of last checkpoint block
