...
__kernel void argon2d_initialize(
	__global struct block* memory, 
        ...
        uint segment_blocks)
{
    ...
    uint buf = 0;
    for (uint i=0; i<8; i++){
        buf ^= memLane[get_local_id(0)+256*i];
    }
    memLane[get_local_id(0)] = buf;
}
void g_shuffle(
	const uint32_t r, 
	__local uint64_t* a, 
...
__kernel void argon2d_finalize(
	__global struct block* memory,
	__global uint32_t* output,
	__local uint32_t* smem,
	...
	)
{
	...
	__local uint32_t* input = &smem[290*get_local_id(1)];
	__local uint64_t* buffer= (__local uint64_t*)&smem[290*get_local_size(1)+get_local_id(1)*36];
	__local uint64_t* input_64 = (__local uint64_t*)input;
	load_block_fin(memLane,&input[1],idx);
	if ( idx == 0){
		input[0]=32;
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	...
	blake2b_compress(&state,&input_64[0],buffer,1,idx);
	blake2b_compress(&state,&input_64[16],buffer,2,idx);
	blake2b_compress(&state,&input_64[32],buffer,3,idx);
	blake2b_compress(&state,&input_64[48],buffer,4,idx);
	blake2b_compress(&state,&input_64[64],buffer,5,idx);
	blake2b_compress(&state,&input_64[80],buffer,6,idx);
	blake2b_compress(&state,&input_64[96],buffer,7,idx);
	blake2b_compress(&state,&input_64[112],buffer,8,idx);
	zero_buffer(&input[257], idx);
	blake2b_compress_final(&state,&input_64[128],buffer,9,idx);
	barrier(CLK_LOCAL_MEM_FENCE);
	if ( state.a <= target && idx==3) {
		 output[0] = SWAP4(nonce);
	}
}