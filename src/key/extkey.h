...
public:
    ...
    CExtKeyPair(CExtKey &vk)
    {
        ...
        nChild = vk.nChild;
        memcpy(chaincode.begin(), vk.chaincode.begin(), 32);
        key = vk.key;
    ...
    CExtKeyPair(CExtPubKey &pk)
    {
        ...
        nChild = pk.nChild;
        memcpy(chaincode.begin(), pk.chaincode.begin(), 32);
        key.Clear();
        pubkey = pk.pubkey;
    };
    ...
    CExtKey GetExtKey() const
    {
        ...
        vk.nChild = nChild;
        memcpy(vk.chaincode.begin(), chaincode.begin(), 32);
        vk.key = key;
        return vk;
    };
...
public:
    ...
    isminetype IsMine() const
    {
        ...
        if ((nFlags & EAF_HARDWARE_DEVICE)) {
            return (isminetype)((int)ISMINE_SPENDABLE);
        }
