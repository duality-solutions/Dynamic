...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const Consensus::Params& consensusParams, const std::atomic<bool>& interruptMsgProc)
{
    ...
    */
    ...
    /* ASSET END */
        ...
        if (!AlreadyHave(inv) && 
            AcceptToMemoryPool(mempool, state, ptx, &fMissingInputs, &lRemovedTxn, false /* fOverrideMempoolLimit */, 0 /* nAbsurdFee */)) {
            ...
            while (!vWorkQueue.empty()) {
                ...
                for (auto mi = itByPrev->second.begin();
                    ...
                    if (setMisbehaving.count(fromPeer))
                        continue;
                    if (AcceptToMemoryPool(mempool, stateDummy, porphanTx, &fMissingInputs2, &lRemovedTxn, false /* fOverrideMempoolLimit */, 0 /* nAbsurdFee */)) {
