...
bool ValidatePaymentOptionsMask(const uint64_t &paymentOptionsMask) {
	uint64_t maxVal = PAYMENTOPTION_DYN | PAYMENTOPTION_SEQ | PAYMENTOPTION_BTC;
	return !(paymentOptionsMask < 1 || paymentOptionsMask > maxVal);
}

bool IsValidPaymentOption(const uint64_t &paymentOptionsMask) {
	return (paymentOptionsMask == PAYMENTOPTION_DYN || paymentOptionsMask == PAYMENTOPTION_SEQ || paymentOptionsMask == PAYMENTOPTION_BTC);
}
...
bool RevertOffer(const std::vector<unsigned char>& vchOffer, const int op, const uint256 &txHash, sorted_vector<std::vector<unsigned char> > &revertedOffers) {
	...
	if (!pofferdb->ReadLastOffer(vchOffer, dbOffer)) {
		...
		{
			errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 2022 - " + _("Failed to erase offer");
	...
	{
		errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 2022 - " + _("Failed to write to offer DB");
		return error(errorMessage.c_str());
	}
...
bool CheckOfferInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, const std::vector<unsigned char> &vvchAlias, bool fJustCheck, int nHeight, sorted_vector<std::vector<unsigned char> > &revertedOffers, string &errorMessage, bool dontaddtodb) {
	...
	if(fJustCheck)
	{
		...
		{
			errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1009 - " + _("Offer price must be a positive number");
			return error(errorMessage.c_str());
		}
		...
		{
			errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1011 - " + _("Offer currency code too long");
			return error(errorMessage.c_str());
		}
		switch (op) {
		case OP_OFFER_ACTIVATE:
			...
			{
				errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 2004 - " + _("offer hex guid too long");
				return error(errorMessage.c_str());
			}
			...
			{
				errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1016 - " + _("Invalid offer type");
				return error(errorMessage.c_str());
			}
			
			if(theOffer.nCommission > 100 || theOffer.nCommission < -90)
			{
				errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1019 - " + _("Commission percentage must be between -90 and 100");
				return error(errorMessage.c_str());
			}		
			...
			if (IsOfferTypeInMask(theOffer.offerType, OFFERTYPE_AUCTION))
			{
				...
				{
					errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Reserve price must be greator or equal to 0");
					return error(errorMessage.c_str());
				}
				if (theOffer.auctionOffer.fDepositPercentage < 0)
				{
					errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Auction deposit percentage must be greator or equal to 0");
					return error(errorMessage.c_str());
				}
				if (theOffer.auctionOffer.nExpireTime > 0 && theOffer.auctionOffer.nExpireTime < nTime)
				{
					errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 4042 - " + _("Invalid auction expiry");
					return error(errorMessage.c_str());
				}
			}
			break;
		case OP_OFFER_UPDATE:		
			...
			if (!ValidateOfferTypeMask(theOffer.offerType))
			{
				errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1027 - " + _("Invalid offer type");
				return error(errorMessage.c_str());
			}
			...
			if(theOffer.nCommission > 100 || theOffer.nCommission < -90)
			{
				errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1034 - " + _("Commission percentage must be between -90 and 100");
				return error(errorMessage.c_str());
			}
			break;
		default:
	...
	if (!fJustCheck && !dontaddtodb) {
		...
		{
			errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 2028 - " + _("Failed to revert offer");
			return error(errorMessage.c_str());
		}
	}
	...
	if (op == OP_OFFER_UPDATE) {
		...
		{
			errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot edit this offer. Offer owner must sign off on this change.");
			return true;
		}
		...
		if (IsOfferTypeInMask(dbOffer.offerType, OFFERTYPE_AUCTION))
		{
			if (!IsOfferTypeInMask(theOffer.offerType, OFFERTYPE_AUCTION))
			{
				errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Cannot change to this offer type until auction expires");
				return true;
			}
			if (theOffer.auctionOffer.fReservePrice < 0)
			{
			...
			{
				errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Cannot modify auction parameters while it is active. Please wait until auction has expired before updating.");
				return true;
			}
		}
	...
	else if(op == OP_OFFER_ACTIVATE)
	{
		...
		if (!theOffer.vchLinkOffer.empty())
		{
			...
			{
				errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1051 - " + _("Linked offer alias not found. It may be expired");
				return true;
			}
			if (theAlias.offerWhitelist.GetLinkEntryByHash(vvchAlias, entry))
			{
				...
				{
					throw runtime_error("DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1514 - " + _("This resold offer must be of higher price than the original offer including any discount"));
				}
			}
			// make sure alias exists in the root offer affiliate list
			else
			{
				throw runtime_error("DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1515 - " + _("Cannot find this alias in the parent offer affiliate list"));
			}

			if (!linkOffer.vchLinkOffer.empty())
			{
				throw runtime_error("DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1516 - " + _("Cannot link to an offer that is already linked to another offer"));
			}
			else if (linkOffer.sCategory.size() > 0 && boost::algorithm::istarts_with(stringFromVch(linkOffer.sCategory), "wanted"))
			{
				throw runtime_error("DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1517 - " + _("Cannot link to a wanted offer"));
			}
	...
	if(op == OP_OFFER_UPDATE)
	{
		...
		if(!theOffer.vchLinkOffer.empty())
		{
			if (!GetOffer(theOffer.vchLinkOffer, linkOffer))
			{
				errorMessage = "DYNAMIC_OFFER_CONSENSUS_ERROR: ERRCODE: 1096 - " + _("Linked offer not found. It may be expired");
				return true;
			}
			theOffer.nQty = linkOffer.nQty;
			theOffer.vchCert = linkOffer.vchCert;
...
UniValue offernew(const UniValue& params, bool fHelp) {
	...
	if (!GetAlias(vchAlias, alias))
		throw runtime_error("DYNAMIC_OFFER_RPC_ERROR ERRCODE: 1500 - " + _("Could not find an alias with this name"));
	...
	{
		string err = "DYNAMIC_OFFER_RPC_ERROR ERRCODE: 1504 - " + _("Could not validate offer type");
		throw runtime_error(err.c_str());
	}
	...
	if(!vchCert.empty())
	{
		...
		{
			throw runtime_error("DYNAMIC_OFFER_RPC_ERROR: ERRCODE: 1508 - " + _("Cannot create this offer because the certificate alias does not match the offer alias"));
		}
	}
	else if(boost::algorithm::istarts_with(stringFromVch(vchCategory), "certificates"))
	{
	...
	scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_OFFER) << CScript::EncodeOP_N(OP_OFFER_ACTIVATE) << vchHashOffer << OP_2DROP << OP_DROP;
	scriptPubKey += scriptPubKeyOrig;
	CScript scriptPubKeyAlias;
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_DYNAMIC_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << alias.vchAlias << alias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
...
UniValue offerlink(const UniValue& params, bool fHelp) {
	...
	CAliasIndex alias;
	if (!GetAlias(vchAlias, alias))
		throw runtime_error("DYNAMIC_OFFER_RPC_ERROR ERRCODE: 1510 - " + _("Could not find an alias with this name"));
	...
	if (vchLinkOffer.empty() || !GetOffer( vchLinkOffer, linkOffer))
		throw runtime_error("DYNAMIC_OFFER_RPC_ERROR ERRCODE: 1513 - " + _("Could not find offer with this guid"));
...
UniValue offerupdate(const UniValue& params, bool fHelp) {
	...
	if (!GetOffer( vchOffer, theOffer))
		throw runtime_error("DYNAMIC_OFFER_RPC_ERROR ERRCODE: 1534 - " + _("Could not find offer with this guid"));
	...
	if (!GetBoolArg("-unittest", false)) {
		...
		for (auto& arrivalTime : arrivalTimes) {
			...
			if ((nNow - (arrivalTime.second / 1000)) < ZDAG_MINIMUM_LATENCY_SECONDS) {
				throw runtime_error("DYNAMIC_OFFER_RPC_ERROR: ERRCODE: 2510 - " + _("Please wait a few more seconds and try again..."));
			}
		}
	}

	if (!GetAlias(theOffer.vchAlias, offerAlias))
		throw runtime_error("DYNAMIC_OFFER_RPC_ERROR ERRCODE: 1535 - " + _("Could not find an alias with this name"));

	if (!GetAlias(vchAlias, alias))
		throw runtime_error("DYNAMIC_OFFER_RPC_ERROR ERRCODE: 1536 - " + _("Could not find an alias with this name"));
	...
	if(!strCert.empty())
	{
		...
		{
			throw runtime_error("DYNAMIC_OFFER_RPC_ERROR: ERRCODE: 1539 - " + _("Cannot update this offer because the certificate alias does not match the offer alias"));
		}
    ...
    vector<unsigned char> vchHashOffer = vchFromValue(hash.GetHex());
	scriptPubKey << CScript::EncodeOP_N(OP_DYNAMIC_OFFER) << CScript::EncodeOP_N(OP_OFFER_UPDATE) << vchHashOffer << OP_2DROP << OP_DROP;
	scriptPubKey += scriptPubKeyOrig;
	...
	vecSend.push_back(recipient);
	CScript scriptPubKeyAlias;
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_DYNAMIC_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << offerAlias.vchAlias << offerAlias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	scriptPubKeyAlias += scriptPubKeyOrig;
	CRecipient aliasRecipient;
	CreateRecipient(scriptPubKeyAlias, aliasRecipient);
	CRecipient aliasPaymentRecipient;
...
UniValue offerinfo(const UniValue& params, bool fHelp) {
	...
	if (!pofferdb || !pofferdb->ReadOffer(vchOffer, txPos))
		throw runtime_error("DYNAMIC_OFFER_RPC_ERROR: ERRCODE: 5536 - " + _("Failed to read from offer DB"));
...
{
	if (paymentOption == PAYMENTOPTION_DYN)
		return CChainParams::ADDRESS_DYN;
	else if (paymentOption == PAYMENTOPTION_SEQ)
		return CChainParams::ADDRESS_SEQ;
