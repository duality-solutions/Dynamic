...
QString TransactionDesc::FormatTxStatus(const CWalletTx& wtx)
{
    ...
    if (!CheckFinalTx(wtx)) {
        ...
        if (!instantsend.HasTxLockRequest(wtx.GetHash()))
            return strTxStatus; // regular tx
...
QString TransactionDesc::toAssetHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    if (nAssetsRec > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + DynamicUnits::formatWithCustomName(QString::fromStdString(rec->assetName), nAssetsRec, rec->units) + "<br>";
    } else {
        strHTML += "<b>" + tr("Debit") + ":</b> " + DynamicUnits::formatWithCustomName(QString::fromStdString(rec->assetName), nAssetsRec, rec->units, true) + "<br>";
    }

    strHTML += "<b>" + tr("Net DYN amount") + ":</b> " + DynamicUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal dynamic:URI (dynamic:123...?message=example)
...
/* BCLog/logCategories not implemented yet
    //
    // Debug view
    //
        ...
        if (wallet->IsMine(txin)) {
            ...
            if (assetData.nAmount > 0) {
                strHTML += "<b>" + tr("Debit") + ":</b> " +
                           DynamicUnits::formatWithCustomName(QString::fromStdString(assetData.assetName), -assetData.nAmount) + "<br>";
            }
            strHTML += "<b>" + tr("Debit") + ":</b> " +
                       DynamicUnits::formatHtmlWithUnit(unit, -debit) + "<br>";
        }
    ...
    for (const CTxOut& txout : wtx.tx->vout)
        if (wallet->IsMine(txout)) {
            if (txout.scriptPubKey.IsAssetScript()) {
                ...
                strHTML += "<b>" + tr("Credit") + ":</b> " +
                           DynamicUnits::formatHtmlWithUnit(unit, wallet->GetCredit(txout, ISMINE_ALL)) + "<br>";
        }
