...
UniValue swapdynamic(const JSONRPCRequest& request)
{
    ...
    if (vchAddress.size() - 34 > 0)
    {
        ...
        std::string strRemoveCheckSumHex = strTypeHex + strPubKeyHex;
        std::vector<uint8_t> vchRemoveCheckSum = ParseHex(strRemoveCheckSumHex);
        std::string strPreimage = "SS58PRE" + strSubstrateAddress.substr(0, 45);
        std::vector<uint8_t> vchData = vchFromString(strPreimage);
        std::string strPreimageBase58 = EncodeBase58(vchRemoveCheckSum);
        oResult.push_back(Pair("nochecksum_hex", strRemoveCheckSumHex));
...
static const CRPCCommand commands[] =
    {
        ...
        {"fluid", "swapdynamic", &swapdynamic, true, {"address", "amount"}},
        {"fluid", "getswap", &getswap, true, {"address_hex"}},
#endif //ENABLE_WALLET
