...
BOOST_AUTO_TEST_CASE(GetHashGPU_check)
{
    ...
    gpu::ProcessingUnit processingUnit(&context, &params, &device, 1, false, false);
    ...
    uint256 gpuHash;
    processingUnit.setInputAndSalt(0, (const void*)input, INPUT_BYTES);
    processingUnit.beginProcessing();
    processingUnit.endProcessing();
    processingUnit.getHash(0, (uint8_t*)&gpuHash);

    BOOST_CHECK(cpuHash == gpuHash);
