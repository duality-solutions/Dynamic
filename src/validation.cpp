...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (IsPoS)
        LogPrintf("%s: Proof of stake found. block=%s, sig-size = %d, signature %s, vtx.size() %d\n", __func__, block.GetHash().ToString(), block.vchBlockSig.size(), EncodeBase64(&block.vchBlockSig[0], block.vchBlockSig.size()), block.vtx.size());
